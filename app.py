...
def add_comment():
    search = request.get_json()
    Storage.add_comment(Comment(None, search['comment_text'], search['task_id']))
    return redirect(url_for('home'))
...
def get_comments():
    search = request.get_json()
    comments = Storage.get_comments(search['task_id'])
    commentsList = []
    if comments:
        for com in comments:
            commentsList.append(Comment(com[0], com[1], com[2]).serialize())
...
def update_task():
    search = request.get_json()
    Storage.update_task(search['task_id'], search['header_text'], search['description_text'])
    return jsonify(search)
...
def filter():
    search = request.get_json()
    tasks = Storage.filter(search['state'], search['title'], session['user_id'])
    taskList = []
    for t in tasks:
        taskList.append(Task(t[0], t[1], t[2], t[3]).serialize())
    return json.dumps(taskList)
...
if __name__ == '__main__':
    app.env = 'development'
    app.run(port=5597, host='127.0.0.1', debug=True)
